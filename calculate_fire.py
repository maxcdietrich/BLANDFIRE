...
def calculate_fire(start_tick, tick_limit, previous_burning_cells, previous_extinguished_cells, test_map):
    ...
    if start_tick >= tick_limit:
        ...
        for cell in previous_burning_cells:
            ...
            new_burning_cells = set(catch_on_fire(cell, test_map))
            current_burning_cells |= new_burning_cells
...
def run_model(tick_limit):
    ...
    burning_cells = [(400,500)]
    extinguished_cells = []
    calculate_fire(0, tick_limit, burning_cells, extinguished_cells, test_map) #DO NOT PASS IN map AS A PARAMETER
