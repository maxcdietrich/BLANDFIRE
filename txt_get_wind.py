...
myshp = open("SHAPEFILES/HOUR1/yosemite_landscape_12-03-2019_0900_120.shp", "rb")
mydbf = open("SHAPEFILES/HOUR1/yosemite_landscape_12-03-2019_0900_120.dbf", "rb")
...
# Make the list of four tuples to use as bounds in remap_interval
    ...
    def get_wind_speeds(self):
        ...
        for n in range(len(r)):
            speeds.append(self.records[n].as_dict()['speed'])
            directions.append(self.records[n].as_dict()['QGIS_dir'])
    ...
    def regrid(self, new_size, input_lower_lon, input_upper_lon, input_lower_lat, input_upper_lat):
        ...
        new_grid = [0] * int(len(wind_data)) * int(scaling_factor)

        for id, el in enumerate(new_grid):
            new_grid[id] = wind_data[int(id/scaling_factor)]
